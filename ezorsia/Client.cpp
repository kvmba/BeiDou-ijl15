...
void Client::MoreHook() {
	...
	//   => 最大值 7，范围 0..7。（地面 foothold 的 page 同样是 0..7）
	// 玩家基准层是 7（m_bActive 时；否则 2），正好与最高绳索同层，同层时绘制
	// 顺序取决于入层先后，bot 后加入就被排在绳索后面 —— 这就是"部分绳索被遮"。
	//
	// 因此只需【打破同层平局】即可，抬 1 个层单位（z += 30000）：
	//   +2（z+=60000）实测会让 bot 盖过相邻层的护栏/前景物件；
	//   +1 仅越过与自身同层的对象，不足以压过 page+1 的护栏。
	Memory::WriteInt(0x0092FD49, 0xC000EA56); // 原值 0xC0007526，z + 30000 (1 层单位)
